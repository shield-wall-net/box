...
apt update
apt -y --upgrade install openssl python3 wget gpg lsb-release apt-transport-https ca-certificates gnupg curl
...
then
  useradd "$USER" --shell /bin/bash --home-dir "$DIR_HOME" --create-home --uid "$UID"
fi
chmod 0750 "$DIR_HOME"
